...
const YearlyTopCategoriesDualBar = ({ selectedYear }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `${API_BASE}/api/admin/reports/yearly-top-categories`,
          {
  ...
  const baseOptions = {
    ...
    scales: {
      ...
      y: {
        ...
        ticks: { font: { size: 11 }, callback: (v) => formatInt(v) },
      },
    },
  };
    ...
    <Card className="h-100">
      <Card.Body>
        <Card.Title className="mb-3">
          Top 5 หมวดหมู่ขายดีของปี (บาท / จำนวนชิ้น)
        </Card.Title>
        ...
        {loading ? (
          ...
          <p className="text-muted text-center">ยังไม่มีข้อมูลปีนี้</p>
        ) : (
...
export default YearlyTopCategoriesDualBar;
