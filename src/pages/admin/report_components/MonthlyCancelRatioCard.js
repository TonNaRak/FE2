...
const MonthlyCancelRatioCard = ({ selectedMonth }) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `${API_BASE}/api/admin/reports/monthly-cancel-ratio`,
          {
      ...
      datasets: [
        {
          ...
          backgroundColor: [
            "rgba(220, 53, 69, 0.85)", // แดง - ยกเลิก
            "rgba(25, 135, 84, 0.85)", // เขียว - สำเร็จ
          ],
  ...
  return (
    <Card className="h-100 w-100">
      <Card.Body className="d-flex flex-column">
        <Card.Title>สัดส่วนการยกเลิกออเดอร์ (รายเดือน)</Card.Title>
        ...
        >
          {loading ? (
            ...
            <p className="text-muted text-center">ยังไม่มีข้อมูลในเดือนนี้</p>
          ) : (
        ...
        {/* {!loading && !err && hasData && (
          <div className="mt-2 small text-muted text-center">
            ยกเลิก {thInt(stats.cancelledCount)} ออเดอร์ · มูลค่า{" "}
            {thMoney(stats.cancelledValue)} | สำเร็จ {thInt(stats.successCount)}{" "}
            ออเดอร์ · มูลค่า {thMoney(stats.successValue)}
          </div>
        )} */}
      </Card.Body>
    </Card>
...
export default MonthlyCancelRatioCard;
