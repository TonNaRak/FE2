...
const ProductStatusChart = ({ selectedCategory }) => {
    ...
    const fetchStatusSummary = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          "https://api.souvenir-from-lagoon-thailand.com/api/admin/dashboard/product-status-summary",
          { headers: { Authorization: `Bearer ${token}` }, params }
        );

        const summary = response.data;
        const activeProducts = summary["วางขาย"] || 0;
