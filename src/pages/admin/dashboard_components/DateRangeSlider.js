...
const DateRangeSlider = ({ onDateRangeChange }) => {
    ...
    useEffect(() => {
        const fetchDateLimits = async () => {
            try {
                ...
                const response = await axios.get('https://api.souvenir-from-lagoon-thailand.com/api/admin/dashboard/date-range-limits', {
                    headers: { Authorization: `Bearer ${token}` }
                });

                const startDate = response.data.minDate ? parseISO(response.data.minDate) : new Date();
    ...
    const startDateLabel = format(addDays(minDate, values[0]), 'd MMM yyyy', { locale: th });
    const endDateLabel = format(addDays(minDate, values[1]), 'd MMM yyyy', { locale: th });
